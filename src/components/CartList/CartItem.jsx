...
export default function CartItem({ items }) {
  ...
  return items.map(({ id, images, tittle, price }) => (
    ...
    ">
      ...
      <AiOutlineMinusCircle size={25} className="hover:fill-lime-700 cursor-pointer"/>
      <span className="font-bold font-mono      
      m-2 sm:m-3 lg:m-4 xl:m-5
      text-base sm:text-lg lg:text-xl">3</span>
      <AiOutlinePlusCircle size={25} className="hover:fill-lime-700 cursor-pointer"/>
      </div>
      <div className="flex items-center"><span className="font-bold font-mono
      text-base sm:text-lg lg:text-xl">125</span><span className="ml-px sm:ml-1 lg:ml-1.5 xl:ml-2">грн</span></div>
      <div><RiDeleteBin6Line size={20} className="hover:fill-lime-700 cursor-pointer"/></div>      
    </li>
  ));
}
